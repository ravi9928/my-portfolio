		...
		<section className="py-6 md:py-8 relative">
			<div className="container mx-auto px-4">
				...
				<p className="text-sm sm:text-base text-gray-400 text-center mb-8 sm:mb-12 max-w-2xl mx-auto px-4">
					Specialized in modern backend development technologies and frameworks
				</p>
									...
									<div className="flex flex-wrap sm:flex-nowrap items-center gap-2">
										...
										<div className="flex flex-wrap sm:flex-nowrap gap-1">
											...
											{skill.name === 'PHP / Laravel / CakePHP' && (
												<span className="px-2 py-0.5 text-xs rounded-full bg-orange-500/10 text-orange-400 border border-orange-500/20">Backend Frameworks</span>
											)}
											{skill.name === 'Angular' && (
												<span className="px-2 py-0.5 text-xs rounded-full bg-green-500/10 text-green-400 border border-green-500/20">Frontend Integration</span>
											)}
											{skill.name === 'JavaScript / TypeScript' && (
												<span className="px-2 py-0.5 text-xs rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20">Full-stack scripting</span>
											)}
										</div>
									</div>
								...
								<div className="hidden lg:block absolute left-full top-0 ml-4 w-48 p-3 rounded-lg bg-gray-800/90 backdrop-blur-sm opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-10 border border-gray-700/50">
									<div className="text-sm space-y-2">
										...
										<div className="space-y-1 text-xs text-gray-400">
											...
											<p>• {Math.round(2.5)} Years Experience</p>
										</div>
									</div>
								</div>
							</div>
							...
							{skills.slice(4).map((skill) => (
								...
								>
									...
									<div className="absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-500">
										{skill.name.includes('Database Management') ? (
